...
# set up to which number we want to generate the numebered list, for now we set this value to 5, we can increase it later
# TAI: Reset limit
...
def splitEdgePunct_software(input):
    # print("TAI splitEdgePunct_software input: ",input)

    # TAI: catastrophic backtracking with line below - NEEDS FIX
    ...
    # print("TAI Func_Name_Recursive_Words: ", Func_Name_Recursive_Words)
...
def Split_End_of_Sentence_Punc(token_list):
    ...
    # print("BEFORE end_token: ", end_token)
    # if token_list[-1][-1]==".":
    if len(end_token) == 1:
...
def Split_Words_Inside_Parenthesis(input_word):
    ...
    if (input_word[0] == "[" and input_word[-1] == "]") or (input_word[0] == "{" and input_word[-1] == "}") or (
        ...
        input_word = input_word.replace("[", " [ ")
        input_word = input_word.replace("]", " ] ")
        # TAI: Added since maybe author forgot?
        # input_word = input_word.replace("(", " ( ")
        # input_word = input_word.replace(")", " ) ")
        new_token = [input_word]
        # print(input_word)
...
if __name__ == '__main__':
    ...
    # text = "In js $http.get().success GNU/Linux 4.2.6-200.fc22.x86_64 you call entityManager.fetchMetadata().then(success, failed) method's first data parameter in templates/your_template/index.php; (Lets check Intel 64)"
    ...
    # TAI: the sentence below falls into Catastrophy Backtracking so never returns output
    # text = "x86-64 (also known as x64, x86_64, AMD64, and Intel 64) is a 64-bit version"
    text = "Improvements10.0.9.348(BlackBerry 10 release version)See BlackBerry 10 for features.10.0.10.672 3rd party app performance"
